...
| `_report/agent/<agent-id>/<plan-id>/evidence*.json` | Evidence-scope surveys backing `evidence_scope.receipts`. | `python -m tools.planner.cli attach-receipt … --file _report/agent/<id>/<plan>/evidence.json`, bespoke scripts that summarise external research. | Plans with `version >= 1` must record at least one survey receipt before moving to `in_progress`. Reference [`docs/reference/evidence-scope.md`](evidence-scope.md) for schema expectations. |
| `_report/usage/evidence-scope/` | Evidence coverage dashboards + `latest.json` pointer produced by the scope CLI. | `python -m tools.planner.evidence_scope --all`, plan hygiene automation. | Each run writes `evidence-scope-<timestamp>.json` summarizing coverage plus `latest.json` so guards/dashboards can detect missing citations at a glance. |
...
| `artifacts/systemic_out/<UTC>/metadata.json` | Tier 1 evaluation provenance (`teof up --eval`, `bin/teof-up`). Records ensemble outputs, Tier 1 receipts, and the git/intent block from the latest quickstart run. | `teof up --eval`, `bin/teof-up`, `python -m tools.autonomy.onboarding_runner`. | Consume this when auditing Tier 1 because it now embeds `quickstart_git` + `quickstart_intent`; no need to scrape `_report/usage/onboarding/quickstart-*.json` unless metadata is missing. |
| `docs/usage/capability-heartbeat/` | Git-tracked heartbeat receipts proving key CLIs still run even when `_report/` is ignored. | `python -m teof plan_scope … --receipt-dir docs/usage/capability-heartbeat`, `python -m teof receipts_stream … --receipt …`, `python -m teof receipts_stream_guard … --receipt …`, `python -m teof scan_history … --receipt-path …`. | Keep each receipt timestamped, maintain `latest.json`, and treat it as the public mirror when `_report/` entries are ignored. |
...
| `_report/analysis/reflection-gap/` | Reflection metadata audits highlighting missing plan IDs or tags on `memory/reflections/*`. | `python3 -m tools.memory.reflection_audit [--window-days N] [--fail-on-missing]`. | Reference in ND-074/behavioral gap plans so missing metadata is tracked; keep `latest.json` under version control or cite from `_report/` receipts. |
