...
| `governance/` | L1 ↔ L2 | Append-only anchors, policy, and key material. | See [`governance/README.md`](../governance/README.md); audit receipts live under `_report/usage/anchors/` until the append-only guard lands. Pattern C design intent: [`docs/foundation/design-intent-pattern-c.md`](../docs/foundation/design-intent-pattern-c.md). Guard tier manifest (`governance/core/guard-tier-manifest.json`) keeps enforcement layering explicit. |
