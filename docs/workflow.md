...
- **P1.2 evidence scope:** before touching shared code, record `evidence_scope` (internal + external + comparative references) and capture a receipt. Plans using `--plan-version 1` plus `planner new --evidence-*` are the supported path; enforce via `teof operator verify --require-evidence-plan <plan>` and `python -m tools.agent.push_ready --require-evidence-plan <plan>`.
...
2) Produce an E2E plan using `docs/quickstart.md` (exact commands, no guessing) and drop it in `_plans/` (`*.plan.json`). Validate with `python3 tools/planner/validate.py`.  
 ...
 2b) When the work depends on external research (most L4+ initiatives), scaffold with `python -m tools.planner.cli new ... --plan-version 1 --evidence-internal ... --evidence-external ...` so `evidence_scope` exists before the first edit. Capture the survey under `_report/agent/<id>/<plan_id>/evidence.json` and enforce it via `--require-evidence-plan`.
  2c) After major plan or backlog updates, run `python -m tools.planner.evidence_scope --all --fail-on-missing --fail-on-missing-receipts` to emit `_report/usage/evidence-scope/evidence-scope-*.json` **and** fail fast when version ≥ 1 plans drift (attach the receipt to your plan or memory log when proposing structural work).
...
10) Keep plan receipts audited: run `python3 scripts/ci/check_plan_receipts_exist.py` regularly and log the summary under `_report/usage/` so missing/untracked evidence is caught early. Use `tools/agent/preflight.sh core|full` to separate observation (core) from workflow (operational) guards; the full lane now runs `python -m tools.planner.evidence_scope --all --fail-on-missing --fail-on-missing-receipts` so every preflight leaves an evidence coverage receipt and blocks when schema-v1 plans drift. Each run drops `_report/usage/preflight/preflight-*.json` so we can monitor how often the heavier lane is needed (`python -m tools.usage.preflight_summary` rolls these receipts up for dashboards).
...
11) Before editing, review active claims via `python3 -m teof bus_status --active-only` (legacy path: `python -m tools.agent.bus_status`) so you coordinate with current owners instead of colliding; escalate on the bus when overlaps appear.  
...
- **Update assignments and claims.** Once a plan exists, create or refresh the bus claim (`python -m teof bus_claim claim --task <task> --plan <plan_id>`) so coordination surfaces know who owns the promoted idea; release or reassign via `tools.agent.task_sync` as work progresses.
- **Enforce shared stewardship.** Before committing or logging progress, run `python3 -m tools.agent.stale_claims --agent <id> --threshold-hours 6 --fail-on-stale` (add `--release` when stepping away) so idle claims never block the lane and any agent can resume the work with receipts already in place.
