...
2) Produce an E2E plan using `docs/quickstart.md` (exact commands, no guessing) and drop it in `_plans/` (`*.plan.json`). Validate with `python3 tools/planner/validate.py`.  
 ...
 4a) Rapid spikes belong in the sandbox lane: `teof-plan new <slug> --exploratory` writes to `_plans/exploratory/` with auto-expiry receipts under `_report/exploratory/`; run `python -m tools.planner.exploratory_lane --receipt` during hygiene sweeps to capture lane status, and promote into the canonical lane once the experiment stabilizes.  
